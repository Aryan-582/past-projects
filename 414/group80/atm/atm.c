...
{
    unsigned char ciphertext[data_len + 17];
    int ciphertext_len;
    ...
    unsigned char *output = encode(ciphertext, ciphertext_len);
    //printf("Encoded message %s of length %ld\n", output, strlen((char *)output));
    output[strlen((char *) output)] = '\0';

    // Returns the number of bytes sent; negative on error
    return sendto(atm->sockfd, (char *) output, strlen((char *) output), 0,
...
ssize_t atm_recv(ATM *atm, char *data, size_t max_data_len)
{
    ...
    recvfrom(atm->sockfd, buffer, max_data_len, 0, NULL, NULL);
    //printf("Received from bank %ld chars: <START>%s<END>\n", strlen(buffer), buffer);
    unsigned char *ciphertext = decode(buffer, strlen(buffer));
    //printf("Ciphertext length: %ld\n", strlen((char *) ciphertext));
    // Returns the number of bytes received; negative on error
    return decrypt(ciphertext, strlen((char *)ciphertext), atm->key, atm->iv, (unsigned char *) data);
}
...
void begin_session(ATM *atm, char *name)
{
    ...
    unsigned char *pin_ciphertext = decode(card, strlen(card));
    int pin_ciphertext_len = strlen((char *) pin_ciphertext);
