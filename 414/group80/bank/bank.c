...
{
    //printf("Encrypting response: <START>%s</END>\n", data);
    ...
    return sendto(bank->sockfd, (char *) output, strlen((char *)output), 0,
...
ssize_t bank_recv(Bank *bank, char *data, size_t max_data_len)
{
    ...
    recvfrom(bank->sockfd, buffer, max_data_len, 0, NULL, NULL);
    ...
    return decrypt(ciphertext, strlen((char *) ciphertext), bank->key, bank->iv, (unsigned char *) data);
}
...
void bank_process_remote_command(Bank *bank, char *command, size_t len)
{
    ...
    {
	//printf("String to process balance: <>%s</>", string);
