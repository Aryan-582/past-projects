...
def parse_fooABC(data, balfound, transfound, invfound, balancecipher, transcipher, invcipher, msglist, accounts, msgindex, datalist, replacetrans):
    ...
    if balfound == 0:
        ...
        if transfound == 0:
            ...
            msgindex += 1
            replacetrans.append(data[32:48])
...
try:
    ...
    transfer = finaldata[finalidx]
    construct.append(transfer[0:32])
    construct.append(transfer[48:80])
    for elem in replacefin:
        if elem != transfer[32:48]:
            construct.append(elem)
            break
except Exception as e: print("no unique account num")
finalreplace = construct[0] + construct[2] + construct[1]   
with open('task3.out', 'wb') as f:    
   for elem in finaldata:
    if elem == finaldata[finalidx]:
