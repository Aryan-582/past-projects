...
public class TubeVideosManager implements TubeVideosManagerInt {
	...
	public ArrayList<Video> getAllVideosInDB() {
		...
		for (int i = 0; i < this.videoDatabase.size(); i++) {
			temp.add(this.videoDatabase.get(i));
		}
		return temp;
	}
		...
		if (title == null || title.isBlank()) {
			...
			for (int i = 0; i < this.videoDatabase.size(); i++) {
				if (this.videoDatabase.get(i).getTitle().equals(title)) {
					Video copy = this.videoDatabase.get(i);
					return copy;
				}
			}
			return null;
	...
	public String[] getPlaylistsNames() {
		if (this.playlists.size() == 0) {
			...
			String[] temp = new String[this.playlists.size()];
			for (int i = 0; i < this.playlists.size(); i++) {
				temp[i] = this.playlists.get(i).getName();
			}
			return temp;
		}
	}
	...
	public Playlist getPlaylist(String playlistName) {
		if (playlistName == null || playlistName.isBlank()) {
			throw new IllegalArgumentException("invalid paramtererererererererererereres");
		} else {
			Playlist temp = new Playlist(findPlayListInternal(playlistName));
			return temp;
		}
	}
	...
	public Playlist searchForVideos(String playlistName, String title, int maximumDurationInMinutes, Genre genre) {
		Playlist searched = new Playlist(playlistName);
		
		if (title == null && maximumDurationInMinutes < 0 && genre == null) {
			for (int i = 0; i < this.videoDatabase.size(); i++) {
				searched.addToPlaylist(this.videoDatabase.get(i).getTitle());
			}
		
		} else if (title == null && maximumDurationInMinutes > 0 && genre == null) {
			for (int i = 0; i < this.videoDatabase.size(); i++) {
				if (this.videoDatabase.get(i).getDurationInMinutes() <= maximumDurationInMinutes) {
					searched.addToPlaylist(this.videoDatabase.get(i).getTitle());
				}
			}
		} else if (title == null && maximumDurationInMinutes < 0 && genre instanceof Genre) {
			for (int i = 0; i < this.videoDatabase.size(); i++) {
				if (this.videoDatabase.get(i).getGenre().equals(genre)) {
					searched.addToPlaylist(this.videoDatabase.get(i).getTitle());
				}
			}	
		} else if (title != null && maximumDurationInMinutes < 0 && genre == null) {
			...
			for (int i = 0; i < this.videoDatabase.size(); i++) {
				if (this.videoDatabase.get(i).getDurationInMinutes() <= maximumDurationInMinutes
						&& this.videoDatabase.get(i).getGenre().equals(genre)) {
					searched.addToPlaylist(this.videoDatabase.get(i).getTitle());
				}
			}
		} else if (title != null && maximumDurationInMinutes < 0 && genre != null) {
			...
			for (int i = 0; i < this.videoDatabase.size(); i++) {
				if (this.videoDatabase.get(i).getDurationInMinutes() <= maximumDurationInMinutes
						&& this.videoDatabase.get(i).getGenre().equals(genre)
						&& this.videoDatabase.get(i).getTitle().equals(title)) {
					searched.addToPlaylist(this.videoDatabase.get(i).getTitle());
				}
			}
		}
