...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  ...
  let m1 =
    ...
    m1
  | Union (a, b) ->  
  let m1 = regexp_to_nfa a in 
  let m2 = regexp_to_nfa b in
  let x = fresh() in
  let y = fresh() in
  let m3 = 
  { qs= List.sort_uniq Stdlib.compare(m1.qs @ m2.qs @ [x; y])
  ; sigma= List.sort_uniq Stdlib.compare(m1.sigma @ m2.sigma)
  ; delta= List.sort_uniq Stdlib.compare(m1.delta @ m2.delta @ [(x, None, m1.q0); (x, None, m2.q0); ((List.hd m1.fs), None, y); ((List.hd m2.fs), None, y)])
  ; q0= x
  ; fs= [y] } in
  m3
  | Concat (a, b) -> 
  ...
  let m2 = regexp_to_nfa b in
  let m3 = 
  { qs= List.sort_uniq Stdlib.compare(m1.qs @ m2.qs)
  ; sigma= List.sort_uniq Stdlib.compare(m1.sigma @ m2.sigma)
  ; delta= List.sort_uniq Stdlib.compare(m1.delta @ m2.delta @ [((List.hd m1.fs), None, m2.q0)])
  ; q0= m1.q0
  ; fs= m2.fs } in
  m3
  | Star a -> 
  ...
  let m1 = regexp_to_nfa a in
  let m2 = 
  { qs= List.sort_uniq Stdlib.compare(m1.qs @ [x;y])
  ; sigma= m1.sigma 
  ; delta= List.sort_uniq Stdlib.compare(m1.delta @ [((List.hd m1.fs), None, y); (x, None, m1.q0); (x, None, y); (y, None, x)])
  ; q0= x
  ; fs= [y] } in  
  m2 
